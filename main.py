...
class CustomDataset(Dataset):
    def __init__(self, images_path):
        ...
        self.image_names = [f for f in listdir(self.images_path) if isfile(join(self.images_path, f))]

    def __len__(self):
        return len(self.image_names)

    def __getitem__(self, idx):
        image = Image.open(self.images_path + self.image_names[idx])

        image = self.transform(image)

        return image, self.image_names[idx].split(".")[0]
...
def interpolate(model, sketch_decoder, epoch):
    # Mudar aqui para não interpolar entre letras diferentes
    samples, labels = next(iter(dataloader))
    samples = samples.view(int(batch_size / 2), 2, *samples.shape[1:])
    labels = labels.view(int(batch_size / 2), 2, *labels.shape[1:])
